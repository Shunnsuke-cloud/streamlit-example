...
def plot_silhouette(X_scaled, labels, n_clusters):
    # Silhouette plot: sklearn example を参照
...
def plot_scatter_pca(X_scaled, labels, selected_cols):
    ...
    if X_scaled.shape[1] >= 2:
        ...
        # もし1次元しかないならそのままプロット（X軸 = 特徴、Y軸 = 0）
...
if uploaded_file is not None:
    ...
    if run_btn:
        ...
        # DataFrame にクラスタ列を追加して表示（元の列は保持）
        df_result = df.copy()
        df_result["Cluster"] = labels
        st.markdown("### 🧩 クラスタリング結果（先頭 20 行）")
        st.dataframe(df_result.head(20))

        # プロット（Silhouette Plot と 散布図）を横並びで表示
        st.markdown("### 📈 可視化")
        fig1 = None
        fig2 = None
        try:
